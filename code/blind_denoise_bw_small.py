...
def parse_input():
    ...
    # noise parameters
    parser.add_argument('--sigma', type=int, default=15, help='noise sigma in range between 10 and 30')
    parser.add_argument('--seed', type=int, default=8, help='random seed')

    # path
    parser.add_argument('--in_path', type=str, default='images/BSD68/gray/test011.png')
    parser.add_argument('--out_path', type=str, default='output/test011_s15_out.png')
    ...
    assert 15 <= opt.sigma <= 30, "supported sigma values: from 10 to 30"

    return opt


def denoise_bw_small_func():
    arch_opt = ArchitectureOptions(rgb=False, small_network=True)
    opt = parse_input()
    ...
    state_file_name0 = 'models/model_state_small_sigma_blind_bw.pt'.format(opt.sigma)
    ...
    print('Blind denoising of a grayscale image with a small network and sigma = {} done, output PSNR = {:.2f}'.format(opt.sigma, psnr_dn))
    sys.stdout.flush()
