...
def parse_input():
    ...
    # path
    parser.add_argument('--in_path', type=str, default='images/astronomical/gray/test/m36.png')
    parser.add_argument('--train_path', type=str, default='images/astronomical/gray/train/m38.png')
...
def external_adaptation_bw_func():
    ...
    if opt.plot:
        ...
        axs1[0, 0].imshow(tensor_to_ndarray_uint8(test_image_c).squeeze(), cmap='gray', vmin=0, vmax=255)
        axs1[0, 0].set_title('Clean')
        axs1[0, 1].imshow(tensor_to_ndarray_uint8(test_image_n).squeeze(), cmap='gray', vmin=0, vmax=255)
        axs1[0, 1].set_title(r'Noisy with $\sigma$ = {}'.format(opt.sigma))
        axs1[1, 0].imshow(tensor_to_ndarray_uint8(test_image_dn1).squeeze(), cmap='gray', vmin=0, vmax=255)
        axs1[1, 0].set_title('Before Adaptation, PSNR = {:.2f}'.format(psnr_dn1))
        axs1[1, 1].imshow(tensor_to_ndarray_uint8(test_image_dn2).squeeze(), cmap='gray', vmin=0, vmax=255)
        axs1[1, 1].set_title('After Adaptation, PSNR = {:.2f}'.format(psnr_dn2))

        fig2, axs2 = plt.subplots(1)
        axs2.imshow(tensor_to_ndarray_uint8(train_image_c).squeeze(), cmap='gray', vmin=0, vmax=255)
