...
def parse_input():
    ...
    # path
    parser.add_argument('--in_path', type=str, default='images/astronomical/color/test/m36.png')
    parser.add_argument('--train_path', type=str, default='images/astronomical/color/train/m38.png')
    ...
    parser.add_argument('--epochs_between_check', type=int, default=10)
...
def external_adaptation_rgb_func():
    ...
    train_image_c = load_image_from_file(opt.train_path)
    ...
    if opt.plot:
        ...
        axs1[0, 0].imshow(tensor_to_ndarray_uint8(test_image_c).squeeze(), vmin=0, vmax=255)
        axs1[0, 0].set_title('Clean')
        axs1[0, 1].imshow(tensor_to_ndarray_uint8(test_image_n).squeeze(), vmin=0, vmax=255)
        axs1[0, 1].set_title(r'Noisy with $\sigma$ = {}'.format(opt.sigma))
        axs1[1, 0].imshow(tensor_to_ndarray_uint8(test_image_dn1).squeeze(), vmin=0, vmax=255)
        axs1[1, 0].set_title('Before Adaptation, PSNR = {:.2f}'.format(psnr_dn1))
        axs1[1, 1].imshow(tensor_to_ndarray_uint8(test_image_dn2).squeeze(), vmin=0, vmax=255)
        axs1[1, 1].set_title('After Adaptation, PSNR = {:.2f}'.format(psnr_dn2))

        fig2, axs2 = plt.subplots(1)
        axs2.imshow(tensor_to_ndarray_uint8(train_image_c).squeeze(), vmin=0, vmax=255)
        axs2.set_title('Training')

        plt.draw()
        plt.pause(1)

    if opt.save:
        imageio.imwrite(opt.out_path + 'before_adaptation_c.png', tensor_to_ndarray_uint8(test_image_dn1).squeeze())
        imageio.imwrite(opt.out_path + 'after_adaptation_c.png', tensor_to_ndarray_uint8(test_image_dn2).squeeze())
        model_state = {'state_dict': nl_denoiser.patch_denoise_net.state_dict()}
        torch.save(model_state, opt.out_path + 'model_state_after_adaptation_sigma_{}_c.pt'.format(opt.sigma))
