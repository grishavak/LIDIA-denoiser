...
def parse_input():
    ...
    # path
    parser.add_argument('--in_path', type=str, default='images/brick_house/color/test1_color.png')
...
def internal_adaptation_rgb_func():
    ...
    if opt.plot:
        ...
        axs[0, 0].imshow(tensor_to_ndarray_uint8(test_image_c).squeeze(), vmin=0, vmax=255)
        axs[0, 0].set_title('Clean')
        axs[0, 1].imshow(tensor_to_ndarray_uint8(test_image_n).squeeze(), vmin=0, vmax=255)
        axs[0, 1].set_title(r'Noisy with $\sigma$ = {}'.format(opt.sigma))
        axs[1, 0].imshow(tensor_to_ndarray_uint8(test_image_dn1).squeeze(), vmin=0, vmax=255)
        axs[1, 0].set_title('Before Adaptation, PSNR = {:.2f}'.format(psnr_dn1))
        axs[1, 1].imshow(tensor_to_ndarray_uint8(test_image_dn2).squeeze(), vmin=0, vmax=255)
        axs[1, 1].set_title('After Adaptation, PSNR = {:.2f}'.format(psnr_dn2))
        plt.draw()
        plt.pause(1)

    if opt.save:
        imageio.imwrite(opt.out_path + 'before_adaptation_color.png', tensor_to_ndarray_uint8(test_image_dn1).squeeze())
        imageio.imwrite(opt.out_path + 'after_adaptation_color.png', tensor_to_ndarray_uint8(test_image_dn2).squeeze())
