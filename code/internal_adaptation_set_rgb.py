...
def parse_input():
    ...
    # path
    parser.add_argument('--in_path', type=str, default='images/urban100/color/')
...
def internal_adaptation_set_rgb_func():
    ...
    for im_name in image_names:
        ...
        print('Denoising color {} with sigma = {} done.'
        ...
        if opt.save:
            before_adaptation = opt.out_path + im_name[:-4] + '_before_adaptation_color.png'.format(opt.sigma)
            imageio.imwrite(before_adaptation, tensor_to_ndarray_uint8(test_image_dn1).squeeze())
            after_adaptation = opt.out_path + im_name[:-4] + '_after_adaptation_color.png'.format(opt.sigma)
            ...
            state_file = opt.out_path + 'model_state_' + im_name[:-4] + '_sigma_{}_c.pt'.format(opt.sigma)
            torch.save(model_state, state_file)

    avg_psnr1 = avg_psnr1 / len(image_names)
    avg_psnr2 = avg_psnr2 / len(image_names)
    print('\nDenoising a color set with sigma = {} done'.format(opt.sigma))
